package com.platform.application.sysmanage.role.service.impl;
...
@Service
@Transactional
public class TmRoleServiceImpl extends AbstractService implements TmRoleService {
	/**
	 * 日志记录器.
	 */
	private static final Log LOGGER = LogFactory.getLog(TmRoleServiceImpl.class);
	...
	public void stopOrStartRole(final String roleCode, final boolean isStart, final String stopReason) {
		...
		try {
			...
			if (isStart) {
				entity.setStatus("0"/*ManageModelConstants.DIC_ROLE_STATUS_START*/);
				entity.setStartTime(new Date());
				entity.setStopTime(null);
			} else {
				entity.setStatus("1"/*ManageModelConstants.DIC_ROLE_STATUS_STOP*/);
